...
export default function MusicProvider({ children }) {
  ...
  useEffect(() => {
    if (isExpanded) {
      ...
      const focusable = expandedContainerRef.current?.querySelectorAll(
        'button, [tabindex="0"]',
      );
      ...
      const handleKeyDown = (e) => {
        ...
        if (e.key === "Tab") {
          const focusableElements =
            expandedContainerRef.current.querySelectorAll(
              'button, [tabindex="0"]',
            );
    ...
    const fetchLikeStatus = async () => {
      ...
      try {
        const res = await fetch(
          `${BASE_URL}/api/v1/feedback/${currentTrack._id}`,
          {
            headers: {
              "X-API-Key": API_KEY,
            },
        ...
        {currentTrack && (
          <div
            ...
            className={`fixed inset-x-0 flex flex-col items-center transition-all duration-500 ease-out ${
              isExpanded
                ? "bottom-0 h-screen z-[1200]"
                : "bottom-16 h-24 px-4 z-[900]"
            }`}
          >
            {isExpanded && (
              <div
            ...
            >
              ...
              <div
                ...
                aria-label={
                  !isExpanded ? `Open player: ${currentTrack.title}` : undefined
                }
                className={`
                ...
                <div className="px-8 flex-1 flex flex-col justify-center">
                  ...
                  <div className="flex items-center justify-between gap-4 mb-8">
                    ...
                    <div className="min-w-0 text-center flex-1">
                      <h2 className="text-2xl font-black text-white mb-1 truncate">
                        {currentTrack.title}
                      </h2>
                    ...
                    <button
                      ...
                      aria-label={
                        liked ? "Unlike this track" : "Like this track"
                      }
                  ...
                  <div className="flex items-center justify-between px-2 pb-12">
                    <button
                      ...
                      className="text-white/70 hover:text-white transition-colors focus-visible:ring-2 focus-visible:ring-white rounded-full outline-none p-3"
                    >
